...
<head>
    ...
    <title>HonestPick - 문의 관리</title>
    <link rel="stylesheet" href="${path}/_css/admin_home.css">
...
<body>
            ...
            <div class="page-header">
                <div class="header-content">
                    <div class="header-left">
                        ...
                        <p class="page-subtitle">문의를 관리하고 기간별 통계를 확인할 수 있습니다.</p>
                    </div>
                </div>
            </div>
                ...
                <div class="data-table-container">
                        ...
                        <table class="data-table" id="adsTable">
                            <thead>
                                <tr>
                                    ...
                                    <th class="action-col">&nbsp; &nbsp; 확인여부</th>
                                </tr>
                            </thead>
                            <tbody id="adsTableBody">
                                <c:choose>
                                    <c:when test="${empty listInquiry or fn:length(listInquiry) == 0}">
                                        <tr>
                                            <td colspan="8" style="text-align:center; padding: 40px;">
                                                <div style="color: var(--admin-text-light);">
                                                    <i class="fas fa-headset menu-icon" style="font-size: 3rem; margin-bottom: 15px; opacity: 0.5;"></i>
                                                    <p style="font-size: 1.1rem; margin-bottom: 5px;">등록된 문의가 없습니다.</p>
                                                    <p style="font-size: 0.9rem;">새 문의를 추가해보세요.</p>
                                                </div>
                                            </td>
                                        </tr>
                                    </c:when>
                                    <c:otherwise>
                                        <c:forEach var="list" items="${listInquiry}" varStatus="loop">
                                            <tr data-ad-id="${list.inquiryNo}" class="table-row">
                                                <td class="checkbox-col">
                                                    <input type="checkbox" class="ad-checkbox" 
                                                           value="${list.inquiryNo}" 
                                                           onchange="toggleAdSelection(${list.inquiryNo})">
                                                </td>
                                                <td class="number-col">#${list.inquiryNo}</td>
                                                <td><b>${list.memberName}</b></td>
                                                <td><b>${list.inquiryType}</b></td>
                                                <td><b>${list.inquiryContent}</b></td>
                                                <td>${list.inquiryRegidate}</td>
                                                <td class="action-col">
                                                    <div class="action-buttons">
		                                                <c:choose>
		                                                	<c:when test="${list.inquiryCheck == 'NO'}">
		                                                		<button class="action-btn edit" type="button" 
		                                                                onclick="location.href='${path}/Admin_servlet/admin_homeEdit.do?InquiryNo=${list.inquiryNo}'" 
		                                                                title="읽기">
                    ...
                    <div class="pagination-container">
                        <div class="pagination-info" id="paginationInfo">
                            ...
                            <span>
                                <c:choose>
                                    <c:when test="${not empty listInquiry and fn:length(listInquiry) > 0}">
                                        1-${fn:length(listInquiry)} of ${fn:length(listInquiry)} 문의
                                    </c:when>
                                    <c:otherwise>
                                        0-0 of 0 문의
                                    </c:otherwise>
    ...
    <script>
        ...
        function bulkExtend() {
            if (selectedAds.size === 0) {
                showNotification('연장할 문의를 선택해주세요.', 'warning');
                return;
            }
            ...
            if (days && !isNaN(days) && parseInt(days) > 0 && parseInt(days) <= 365) {
                ...
                selectedAds.clear();
                updateBulkActions();
                showNotification('선택된 ' + count + '개 문의의 기간이 ' + days + '일 연장되었습니다.', 'success');
        ...
        function bulkDelete() {
            if (selectedAds.size === 0) {
                showNotification('삭제할 문의를 선택해주세요.', 'warning');
                return;
            }
            
            if (confirm('선택된 ' + selectedAds.size + '개의 문의를 정말 삭제하시겠습니까?\n\n이 작업은 되돌릴 수 없습니다.')) {
        ...
        function exportAds() {
            ...
            try {
                var csvRows = ['문의번호,문의제목,등록일,종료일,남은기간'];
                ...
                link.setAttribute("download", "문의목록_" + new Date().toISOString().split('T')[0] + ".csv");
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                
                showNotification(visibleRows.length + '개의 문의 데이터를 내보냈습니다.', 'success');
            } catch (error) {
